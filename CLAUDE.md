...
Billo is a full-stack web application with a Python Flask API backend and a SolidJS TypeScript frontend, deployed via Docker Compose or Kubernetes. The product helps users manage contacts (organizations and people) and create/download invoices.
...
- `resources/` — Flask-RestX namespaces: `authentication`, `user`, `two_factor`, `workspace`, `billing`, `contacts`, `invoicing`
- `db/` — SQLAlchemy models (workspace, user, billing, contacts, invoicing)
- `tasks/` — Celery task definitions (mail, etc.)
- `app.py` — App factory (registers extensions, blueprints, error handlers)
- `config.py` — `DevConfig`, `ProdConfig`, `TestConfig`; all env vars prefixed `BILLO_`
...
- `locales/` — i18n translation files (add keys to `en.ts` first, run `make add_missing_translations`)
- `validators.ts` — Shared form validation logic
- `form_helpers.ts` — Helpers for `@modular-forms/solid`

### Domain modules
- **Workspaces**: every authenticated action is scoped to a workspace. Members have role `owner`, `admin`, or `member`.
- **Contacts**: per-workspace `Organization` (klanten — companies you invoice) and `Person` (people, optionally linked to an organization).
- **Invoicing**: per-workspace `CompanySettings` (logo, address, default VAT, invoice numbering) and `CustomerInvoice` with line items. Invoices snapshot sender + recipient details at creation time so editing the contact later does not change historical invoices. PDFs are generated server-side with `fpdf2`.

### Configuration
All API config comes from environment variables prefixed `BILLO_`. In development these default to the values in `docker-compose.dev.yml` (DB: `billo_db`, Redis: `localhost:6379`, MailDev SMTP: `localhost:1025`).
